...
export async function createEmbedsPagination(
	...
	reactions: {
		...
		// deno-lint-ignore require-await
		'🗑️': async (setPage, currentPage, pageCount, deletePagination) => deletePagination(),
	},
) {
	if (embeds.length === 0) {
		return;
	...
	await addReactions(embedMessage.channelID, embedMessage.id, Object.keys(reactions), true);
	...
	while (!isEnded) {
		...
		if (embedMessage.guildID) {
			await removeUserReaction(embedMessage.channelID, embedMessage.id, reaction, authorID);
		}
		...
		if (isEnded || !embedMessage || !(await editEmbed(embedMessage, embeds[currentPage - 1]))) {
			return;
		}
	}
}
