...
createCommand({
	...
	execute: async function (message) {
		...
		const currentLanguage =
			allowedLanguages.find((item) => item.id === currentLanguageId) || allowedLanguages[0];
...
createSubcommand('language', {
	...
	execute: async function (message, args: LanguageSetArgs) {
		...
		const oldLanguage =
			allowedLanguages.find((item) => item.id === oldLanguageId) || allowedLanguages[0];
		...
		if (!newLanguage) {
			...
			await db.guilds
				.update(message.guildID, { language: newLanguage.id })
				.catch(console.log);
